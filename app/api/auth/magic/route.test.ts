...
function req(body: unknown, ip?: string) {
  return new Request("http://localhost/api/auth/magic", {
    method: "POST",
    headers: {
...
describe("POST /api/auth/magic", () => {
  it("creates a magic token and sends mail when the email matches an account", async () => {
    mockGetPersonByEmail.mockReturnValue({ id: 7 });
