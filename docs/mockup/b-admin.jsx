...
function AdminInbox({ lang }) {
  ...
  const pending = reservations.filter(
    (r) => r.status === "pending" && (role !== "owner" || caps.ownedCars.includes(r.car))
  );
      ...
      <div style={{ padding: "0 16px" }}>
        {pending.map((r) => {
          ...
          const daysAgo = Math.max(
            0,
            Math.round((new Date("2026-04-21") - new Date(r.requested)) / 86400000)
          );
          const conflict = pending.find(
            (p) => p.id !== r.id && p.car === r.car && !(p.to < r.from || p.from > r.to)
          );
          return (
            <AdminCard key={r.id} stripe={paperB.amber}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <div style={{ flex: 1 }} />
                <AdminButton
                  onClick={() => setMailOpen({ id: r.id, action: "preview" })}
                  variant="ghost"
                  size="sm"
                >
                  ✉ {t.mailPreview}
          ...
          .map((r) => (
            <div
              ...
              style={{
                ...
                gap: 10,
                opacity: 0.8,
                borderLeft: `3px solid ${r.status === "confirmed" ? paperB.green : paperB.accent}`,
              }}
            >
              <CarStamp code={r.car} active size="sm" />
              <div
                style={{ flex: 1, fontFamily: paperB.fontSerif, fontSize: 13, color: paperB.ink }}
              >
                {r.who} ·{" "}
                <span style={{ color: paperB.inkDim, fontFamily: paperB.fontMono, fontSize: 10 }}>
                  {fmtDate(r.from, lang)}
                </span>
              </div>
              <Stamp
                rotate={-4}
                color={r.status === "confirmed" ? paperB.green : paperB.accent}
                size="sm"
              >
                {t[r.status]}
              </Stamp>
...
function MailMockOverlay({ res, action, lang, onClose }) {
  ...
  const subject = isPreview
    ? lang === "nl"
      ? `Nieuwe aanvraag — ${car.name}`
      : `New request — ${car.name}`
    : approved
      ? lang === "nl"
        ? `✓ Reservering bevestigd — ${car.name}`
        : `✓ Reservation confirmed — ${car.name}`
      : lang === "nl"
        ? `✗ Reservering geweigerd — ${car.name}`
        : `✗ Reservation declined — ${car.name}`;
    ...
    >
      ...
      >
        ...
        >
          {lang === "nl" ? (
            <>
              ...
              {isPreview ? (
                <>
                  Er ligt een nieuwe reservering voor <b>{car.name}</b> van{" "}
                  {fmtDate(res.from, lang)}
                  {res.from !== res.to && <> tot {fmtDate(res.to, lang)}</>} — reden: "{res.reason}
                  ".
                  <br />
                  <br />
                ...
                <>
                  ...
                  {res.from !== res.to && <> tot {fmtDate(res.to, lang)}</>} is bevestigd.
                  <br />
                  <br />
                ...
                <>
                  Je reservering voor <b>{car.name}</b> op {fmtDate(res.from, lang)} is helaas niet
                  bevestigd.
                  <br />
                  <br />
            ...
            <>
              ...
              {isPreview ? (
                <>
                  ...
                  {res.from !== res.to && <> to {fmtDate(res.to, lang)}</>} — reason: "{res.reason}
                  ".
                  <br />
                  <br />
                ...
                <>
                  ...
                  {res.from !== res.to && <> to {fmtDate(res.to, lang)}</>} is confirmed.
                  <br />
                  <br />
                ...
                <>
                  Your reservation for <b>{car.name}</b> on {fmtDate(res.from, lang)} could not be
                  confirmed.
                  <br />
                  <br />
        ...
        <div style={{ textAlign: "right", paddingTop: 14 }}>
          <AdminButton onClick={onClose} variant="ghost" size="sm">
            {t.annuleer}
          </AdminButton>
        </div>
      </div>
...
function AdminMembers({ lang }) {
  ...
  const setKl = (name, kl) =>
    setPeople((ps) => ps.map((p) => (p.name === name ? { ...p, kl } : p)));
  const toggleAdmin = (name) =>
    setPeople((ps) => ps.map((p) => (p.name === name ? { ...p, isAdmin: !p.isAdmin } : p)));
    ...
    <div>
      ...
      {inviteInfo && (
        ...
        >
          ...
          >
            {lang === "nl"
              ? `Uitnodiging voor ${inviteInfo.name} · gekopieerd`
              : `Invite for ${inviteInfo.name} · copied`}
          </div>
          <div
            style={{
              fontFamily: paperB.fontMono,
      ...
      <div style={{ padding: "0 16px" }}>
        {people.map((p) => {
          ...
          const ownedCars = CARS.filter((c) => (c.ownerIds || [c.eigenaar]).includes(p.name)).map(
            (c) => c.short
          );
          const isOwner = ownedCars.length > 0;
          const stripeColor = p.isAdmin
            ? paperB.blue
            : isOwner
              ? paperB.amber
              : hasDisc
          ...
          return (
            <AdminCard key={p.name} stripe={stripeColor}>
              ...
              >
                ...
                >
                  {lang === "nl" ? "Admin-rechten" : "Admin rights"}
                </div>
                <button
                ...
                >
                  {p.isAdmin ? "✓ Admin" : lang === "nl" ? "Geen admin" : "Not admin"}
                </button>
              </div>

              <div style={{ display: "flex", gap: 14 }}>
                <DiscountSlider
                  label={t.baseDiscount}
                  value={p.k}
                  onChange={(v) => setK(p.name, v)}
                />
                <DiscountSlider
                  label={t.longDiscountPct}
                  value={p.kl}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div
          style={{
            ...
            fontSize: 12,
            fontWeight: 700,
            color: value > 0 ? paperB.accent : paperB.inkMute,
            minWidth: 38,
            textAlign: "right",
          }}
        >
...
function AdminHygiene({ lang }) {
  ...
  const suspect =
    role === "owner" ? allSuspect.filter((s) => caps.ownedCars.includes(s.fuel.car)) : allSuspect;

  return (
    <div>
      <AdminSectionTitle right={`${gaps.length} ${t.missing}`}>✕ {t.kmGaps}</AdminSectionTitle>
      <div style={{ padding: "0 16px" }}>
        {gaps.length === 0 ? (
          <AdminCard>
            <div
            ...
            >
              {lang === "nl" ? "Geen gaten gevonden ✓" : "No gaps detected ✓"}
            </div>
          </AdminCard>
        ) : (
      ...
      <div style={{ padding: "0 16px" }}>
        {suspect.length === 0 ? (
          <AdminCard>
            ...
            >
              {lang === "nl" ? "Alles ziet er ok uit ✓" : "All looks fine ✓"}
            </div>
          </AdminCard>
        ) : (
...
function KmGapCard({ gap, lang }) {
  ...
  return (
    <AdminCard stripe={paperB.accent}>
      ...
      >
        <div>
          ↑ {gap.after.eind.toLocaleString("nl-BE")} km{" "}
          <span style={{ color: paperB.inkMute }}>({gap.after.who})</span>
        </div>
        <div style={{ color: paperB.accent, fontWeight: 700 }}>? …{gap.missingKm} km gap…</div>
        <div>
          ↓ {gap.before.start.toLocaleString("nl-BE")} km{" "}
          <span style={{ color: paperB.inkMute }}>({gap.before.who})</span>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 4 }}>
        {[null, ...PEOPLE.slice(0, 9)].map((p, i) => {
          ...
          const sel =
            p === null ? assignedTo === null && assignedTo !== undefined : assignedTo === p.name;
          return (
            <button
              key={i}
              onClick={() => setAssignedTo(p === null ? "COOP" : p.name)}
              style={{
                padding: "6px 4px",
                border: `1.5px ${sel ? "solid" : "dashed"} ${paperB.ink}`,
  ...
  return (
    <div>
        ...
        >
          ...
          <input
            ...
            style={{
              ...
              fontFamily: paperB.fontMono,
              fontSize: 12,
              border: `1px solid ${paperB.inkMute}`,
              background: paperB.paper,
              outline: "none",
            }}
          />
      ...
      >
        <span>Σ</span>
        <span>
          {fmtMoney(totalB)} / {fmtMoney(bedrag)}
        </span>
      </div>
    </div>
  );
