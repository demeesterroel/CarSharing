...
function AdminCars({ lang }) {
  ...
  const [carsDraft, setCarsDraft] = React.useState(() =>
    CARS.map((c) => ({ ...c, fixedCosts: (c.fixedCosts || []).map((fc) => ({ ...fc })) }))
  );
  const [helperVariant, setHelperVariant] = React.useState("story"); // 'story' | 'calc'

  const visibleCars = isOwner
    ? carsDraft.filter((c) => caps.ownedCars.includes(c.short))
    : carsDraft;
  ...
  const setCar = (patch) =>
    setCarsDraft((cs) => cs.map((c) => (c.short === selected ? { ...c, ...patch } : c)));
      ...
      <div style={{ padding: "0 16px 12px", display: "flex", gap: 8 }}>
        {visibleCars.map((c) => (
          ...
          >
            ...
            <div style={{ fontSize: 9, marginTop: 3, opacity: 0.75, letterSpacing: 0.5 }}>
              {c.merk}
            </div>
          </button>
        ))}
        ...
        <AdminCard>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                {car.merk} · {car.kleur} · {lang === "nl" ? "eigenaar" : "owner"} {car.eigenaar}
              </div>
            </div>
          </div>
  ...
  return (
    <AdminCard>
      ...
      >
        {lang === "nl" ? (
          <>
            Vorig jaar reed <b>{car.name}</b> <b>{car.jaarKm.toLocaleString("nl-BE")} km</b>. Dat
            kostte de co-op in totaal <b>{fmtMoney(totalCost)}</b> — dus{" "}
            <b>{fmtMoney(totalCost / car.jaarKm)}/km</b> aan echte kosten.
          </>
        ) : (
          <>
            Last year <b>{car.name}</b> drove <b>{car.jaarKm.toLocaleString("nl-BE")} km</b>. Total
            cost was <b>{fmtMoney(totalCost)}</b> — that's{" "}
            <b>{fmtMoney(totalCost / car.jaarKm)}/km</b> real cost.
          </>
        )}
      </div>
      ...
      >
        {(() => {
          const catColors = {
            verzekeringen: paperB.blue,
            belastingen: paperB.ink,
            onderhoud: paperB.amber,
            keuring: paperB.green,
            diversen: paperB.inkDim,
          };
          ...
          const fixedRows = Object.entries(catTotals).map(([cat, yr]) => ({
            l: FIXED_COST_LABELS[cat]?.[lang] || cat,
            v: s.breakdown[cat] || 0,
            yr,
            c: catColors[cat] || paperB.inkDim,
          }));
          return [
        ...
        <div style={{ padding: "0 16px" }}>
          ...
          >
            {lang === "nl" ? (
              scheurt ? (
                <>
                  Met <b>€ {scenario.toFixed(2).replace(".", ",")}/km</b> komen we{" "}
                  <b style={{ color: paperB.accent }}>{fmtMoney(Math.abs(scenarioDelta))} tekort</b>{" "}
                  op een jaar. De eigenaar ({car.eigenaar}) legt zelf bij.
                </>
              ) : (
                <>
                  Met <b>€ {scenario.toFixed(2).replace(".", ",")}/km</b> houdt de co-op{" "}
                  <b style={{ color: paperB.green }}>{fmtMoney(scenarioDelta)} over</b> na een jaar.
                  Ademruimte voor onverwachte kosten.
                </>
              )
            ) : scheurt ? (
              <>
                At <b>€ {scenario.toFixed(2).replace(".", ",")}/km</b> we're{" "}
                <b style={{ color: paperB.accent }}>{fmtMoney(Math.abs(scenarioDelta))} short</b>{" "}
                over a year. Owner ({car.eigenaar}) subsidises.
              </>
            ) : (
              <>
                At <b>€ {scenario.toFixed(2).replace(".", ",")}/km</b> the co-op gets{" "}
                <b style={{ color: paperB.green }}>{fmtMoney(scenarioDelta)} surplus</b> over a
                year.
              </>
            )}
          </div>
      ...
      <div style={{ marginTop: 14 }}>
        <AdminButton>
          ✓{" "}
          {lang === "nl"
            ? `Zet km-prijs op € ${scenario.toFixed(2).replace(".", ",")}`
            : `Set km rate to € ${scenario.toFixed(2).replace(".", ",")}`}
        </AdminButton>
      </div>
    </AdminCard>
  );
...
function KmPriceHelperCalc({ car, lang }) {
  ...
  return (
    <AdminCard>
      ...
      >
        — {t.kmPriceHelper} · {lang === "nl" ? "Rekenmachine" : "Calculator"} —
      </div>
      ...
      >
        {t.breakdown} ({car.jaarKm.toLocaleString("nl-BE")} km)
      </div>
      ...
      >
        ...
        >
          ...
          >
            {scenarioDelta >= 0 ? "+ " : "− "}
            {fmtMoney(Math.abs(scenarioDelta))}
          </span>
        </div>
      </div>
      ...
      <div style={{ marginTop: 14 }}>
        <AdminButton>
          ✓{" "}
          {lang === "nl"
            ? `Zet op € ${scenario.toFixed(2).replace(".", ",")}/km`
            : `Apply € ${scenario.toFixed(2).replace(".", ",")}/km`}
        </AdminButton>
      </div>
    </AdminCard>
  );
    ...
    <div>
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        <AdminCard>
          ...
          {!generated && (
            <div style={{ marginTop: 14 }}>
              <AdminButton onClick={() => setGenerated(true)}>▶ {t.runSettlement}</AdminButton>
            </div>
          )}
        </AdminCard>
        ...
        {generated && (
          <>
            ...
            {rows.map((r) => (
              <SettlementRow key={r.name} row={r} lang={lang} />
            ))}
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {zero ? t.paysZero : credit ? t.payBack : t.toPay}
          </div>
        </div>
        <div
          style={{
      ...
      {expanded && (
        <div style={{ marginTop: 10, animation: "popIn 0.15s ease-out" }}>
          ...
          <div style={{ padding: "4px 0 2px" }}>
            <ReceiptRow
              label={lang === "nl" ? "Ritten (te betalen)" : "Trips (charge)"}
  ...
  return (
    <>
      ...
      <AdminCard>
        ...
        >
          — {lang === "nl" ? "Betaalinstructies" : "Payment instructions"} —
        </div>
        {owners.map((o) => (
          <div
            key={o.owner}
  ...
  return (
    <>
      <AdminCard pad="0">
        ...
        <div
          style={{
            borderTop: `1.5px solid ${paperB.ink}`,
            padding: "12px 14px",
            background: paperB.paperDeep,
          }}
        >
          {owners.map((o) => (
            ...
            >
              ...
              >
                {(o.total >= 0 ? "+ " : "− ") + fmtMoney(Math.abs(o.total))}
              </span>
            </div>
          ))}
        </div>
  ...
  return (
    <>
      ...
      >
        ...
        >
          {(row.ownerNet >= 0 ? "+ " : "− ") + fmtMoney(Math.abs(row.ownerNet))}
        </div>
      </div>
      {open && (
        <div
  ...
  const MOCK_PAYMENTS = [
    {
      ...
      amount: 156.4,
      date: "2026-01-15",
      note: lang === "nl" ? "Afrekening 2025" : "Settlement 2025",
    },
    {
      id: "p2",
      from: "Susanna",
    ...
    },
    {
      ...
      amount: 43.0,
      date: "2026-01-22",
      note: lang === "nl" ? "Terugbetaling 2025" : "Refund 2025",
    },
    {
      id: "p4",
      from: "Tinne",
...
function AdminPage({ lang, setLang }) {
  ...
  const [adminTab, setAdminTab] = React.useState(
    () => localStorage.getItem("cs-admin-tab") || "inbox"
  );
  const [fleetDetail, setFleetDetail] = React.useState(null);

  React.useEffect(() => {
    localStorage.setItem("cs-admin-tab", adminTab);
  }, [adminTab]);
  ...
  React.useEffect(() => {
    if (adminTab !== "fleet") setFleetDetail(null);
  }, [adminTab]);
  ...
  return (
    <div style={{ background: paperB.paperDeep, minHeight: "100%", paddingBottom: 120 }}>
      ...
      {adminTab === "fleet" && detailCar && (
        <BreakEvenDetail car={detailCar} lang={lang} onBack={() => setFleetDetail(null)} />
      )}
