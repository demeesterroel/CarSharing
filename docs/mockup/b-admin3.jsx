...
function BucketCard({ fe, car, lang, onClick, onRaiseRate, compact }) {
  ...
  const projEndText =
    projectedBurden > 0
      ...
      : lang === "nl"
        ? "Break-even is haalbaar dit jaar"
        : "Break-even is within reach this year";
  ...
  return (
    <div
      onClick={!compact && onClick ? undefined : onClick}
      style={{
    ...
    >
      ...
      >
        ...
        <div
          style={{
            ...
            bottom: "calc(" + pct + "% - 2px)",
            height: 3,
            background: past ? paperB.green : paperB.blue,
            opacity: pct > 0 ? 0.7 : 0,
          }}
        />
        <div
        ...
        >
          ...
          >
            {lang === "nl" ? "van" : "of"} {fmtMoney(fe.vasteTotal)}{" "}
            {lang === "nl" ? "gedekt" : "recovered"}
          </div>
        </div>
      </div>
      ...
      >
        <div>
          <span style={{ color: paperB.ink, fontWeight: 700 }}>
            {fe.kmOthers.toLocaleString("nl-BE")}
          </span>
          {" / "}
      ...
      {!compact && lastYearCoopKm > 0 && (
        <div style={{ marginTop: 12 }}>
          ...
          >
            — {lang === "nl" ? "vs. vorig jaar (april)" : "vs. last year (april)"} —
          </div>
          <div
            style={{
              background: paperB.paperDeep,
              padding: "10px 12px",
              border: `1px dashed ${paperB.paperDark}`,
            }}
          >
            <ReceiptRow
...
function AdminFleet({ lang, onOpenCar, onRaiseRate }) {
  ...
  const visibleCars =
    role === "owner" ? CARS.filter((c) => (c.ownerIds || [c.eigenaar]).includes(me)) : CARS;
    ...
    <div>
      ...
      <div style={{ padding: "16px 16px 0" }}>
        ...
        >
          ...
          >
            {totalKmOthers.toLocaleString("nl-BE")} km ·{" "}
            {lang === "nl" ? "door co-op leden gereden" : "driven by co-op members"}
          </div>
        </div>
      </div>
      ...
      >
        <div>
          ■{" "}
          {lang === "nl"
            ? "blauw = vaste kosten worden afbetaald"
            : "blue = fixed costs being recovered"}
        </div>
        <div>
          ▦{" "}
          {lang === "nl"
            ? "groen-gehasht = break-even bereikt, overschot gaat naar eigenaar"
            : "green-hatched = break-even reached, surplus goes to owner"}
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <svg viewBox="0 0 310 162" style={{ width: "100%", display: "block" }}>
        ...
        {last && <circle cx={xOf(last.m)} cy={yOf(last.remaining)} r="4" fill={paperB.accent} />}
      ...
      {contributions.map(({ who, km, recovered }, i) => (
        <div key={who} style={{ marginBottom: i < contributions.length - 1 ? 10 : 0 }}>
          ...
          >
            ...
            >
              {i === 0 && <span style={{ color: paperB.amber }}>★ </span>}
              {who}
            </div>
            <div
              style={{
          ...
          <div
            style={{
              height: 6,
              background: paperB.paperDeep,
              border: `1px solid ${paperB.paperDark}`,
            }}
          >
            <div
              style={{
                height: "100%",
                width: Math.round((km / maxKm) * 100) + "%",
                background: paperB.blue,
              }}
            />
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        <span
          style={{ fontFamily: paperB.fontMono, fontSize: 10, color: paperB.inkMute, minWidth: 38 }}
        >
      ...
      >
        ...
        <ReceiptRow
          ...
          color={
            reachable
              ? paperB.green
              : newBurden > (fe.projectedBurden || 0)
                ? paperB.accent
                : undefined
          }
        />
...
function BreakEvenDetail({ car, lang, onBack }) {
  ...
  const [carDraft, setCarDraft] = React.useState(() => ({
    ...car,
    fixedCosts: (car.fixedCosts || []).map((c) => ({ ...c })),
  }));
    ...
    <div>
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        >
          ...
          >
            {lang === "nl" ? "Wageninstellingen" : "Car settings"}
          </div>
          ...
          >
            {lang === "nl" ? "Jaarlijkse vaste kosten" : "Yearly fixed costs"}
          </div>

          <FixedCostEditor items={carDraft.fixedCosts || []} onChange={setFixedCosts} lang={lang} />
        </div>
      </div>
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        >
          ...
          >
            {lang === "nl" ? "Km-prijs — uit wat bestaat die?" : "Km-rate — what's in it?"}
          </div>
          ...
          >
            ...
            >
              {fmtMoney(fe.variablePerKm).replace("€ ", "€")}
            </div>
            <div
              style={{
                width: fixW + "%",
            ...
            >
              {fmtMoney(fe.fixedContributionPerKm).replace("€ ", "€")}
            </div>
          </div>
          <div
            style={{
          ...
          >
            <div>
              <span style={{ color: paperB.amber, fontSize: 12 }}>■</span>&nbsp;
              {lang === "nl" ? "brandstof + slijtage" : "fuel + wear"}
            </div>
            <div>
              <span style={{ color: paperB.blue, fontSize: 12 }}>■</span>&nbsp;
              {lang === "nl" ? "vaste-kosten bijdrage" : "fixed-cost contribution"}
            </div>
          </div>
          ...
          >
            {lang === "nl" ? (
              <>
                Eigenaar ({fe.ownerIds[0]}) rijdt aan <b>{fmtMoney(fe.variablePerKm)}/km</b>.
                Anderen betalen <b>{fmtMoney(carDraft.prijs)}/km</b> — het verschil vult de emmer.
              </>
            ) : (
              <>
                Owner ({fe.ownerIds[0]}) drives at <b>{fmtMoney(fe.variablePerKm)}/km</b>. Others
                pay <b>{fmtMoney(carDraft.prijs)}/km</b> — the difference fills the bucket.
              </>
            )}
          </div>
        </div>
      ...
      <div style={{ padding: "12px 16px 0" }}>
        ...
        >
          ...
          >
            {lang === "nl" ? "Co-op km · verwacht vs. werkelijk" : "Co-op km · expected vs. actual"}
          </div>
          <div
            style={{
              fontFamily: paperB.fontSerif,
          ...
          >
            {lang === "nl" ? (
              <>
                Verwacht: <b>{fe.expectedCoopKm.toLocaleString("nl-BE")} km</b> dit jaar.
                <br />
                Tot nu toe: <b>{fe.kmOthers.toLocaleString("nl-BE")} km</b> (
                {Math.round(fe.pctOfExpected)}% van verwacht).
              </>
            ) : (
              <>
                Expected: <b>{fe.expectedCoopKm.toLocaleString("nl-BE")} km</b> this year.
                <br />
                So far: <b>{fe.kmOthers.toLocaleString("nl-BE")} km</b> (
                {Math.round(fe.pctOfExpected)}% of expected).
              </>
            )}
          </div>
          <div
          ...
          >
            <div
              style={{
                ...
                top: 0,
                bottom: 0,
                width: Math.min(100, fe.pctOfExpected) + "%",
                background: paperB.ink,
              }}
            />
            <div
          ...
          >
            <div>0 km</div>
            <div>
              {lang === "nl" ? "jaardoel" : "year goal"} ·{" "}
              {fe.expectedCoopKm.toLocaleString("nl-BE")} km
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      >
        ...
        >
          — {lang === "nl" ? "Jaaruitbetaling" : "Year payout"} · {year} —
        </div>
        <div
          style={{
            fontFamily: paperB.fontSerif,
      ...
      {ownerList.map((o) => (
        <OwnerPayoutStory key={o.owner} owner={o} lang={lang} />
      ))}
      ...
      <div style={{ padding: "8px 16px 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <AdminButton size="sm" variant="ghost">
          ✉ {lang === "nl" ? "Mail naar eigenaars" : "Email owners"}
        </AdminButton>
      </div>
    </div>
  );
      ...
      <div style={{ background: paperB.paper, padding: "16px 18px 10px" }}>
          ...
          body={
            lang === "nl" ? (
              <>
                Op{" "}
                {owner.cars.length === 1
                  ? `${owner.cars[0].car.name}`
                  : `je ${owner.cars.length} wagens`}{" "}
                reden co-op-leden dit jaar samen{" "}
                <b>
                  {owner.cars.reduce((s, c) => s + c.fe.kmOthers, 0).toLocaleString("nl-BE")} km
                </b>
              ...
              <>
                ...
                {owner.cars.length === 1
                  ? `${owner.cars[0].car.name}`
                  : `your ${owner.cars.length} cars`}{" "}
                co-op members drove{" "}
                <b>
                  {owner.cars.reduce((s, c) => s + c.fe.kmOthers, 0).toLocaleString("nl-BE")} km
                </b>{" "}
                this year. Each km contributed to your fixed costs.
              </>
            )
          }
        />
        ...
        <StoryAct
          ...
          body={
            lang === "nl" ? (
              <>
                Brandstof betaald, keuring afgerekend, uitlijning bij Donckers — dat wordt nu
                rechtgezet.
              </>
            ) : (
          ...
          {ownerOwnShareFloor > 0.01 ? (
            lang === "nl" ? (
              <>
                ...
                het deel dat overeenkomt met je <b>eigen gereden km</b>. Dat is correct: jij hoort
                jezelf geen huur te betalen.
              </>
            ) : (
              <>
                You still personally carry <b>{fmtMoney(ownerOwnShareFloor)}</b> of fixed costs —{" "}
                the share matching your <b>own km driven</b>. That's correct: you shouldn't pay rent
                to yourself.
              </>
            )
          ) : lang === "nl" ? (
            <>
              De co-op dekte dit jaar al je vaste kosten <b>volledig</b>. Alles wat er bij kwam was
              overschot.
            </>
          ) : (
            <>
          ...
          {owner.cars.map((row) => (
            ...
            >
              ...
              <ReceiptRow
                label={
                  lang === "nl"
                    ? `Emmer-bijdrage (${row.fe.kmOthers.toLocaleString("nl-BE")} km × ${fmtMoney(row.fe.fixedContributionPerKm).replace("€ ", "€")})`
                    : `Bucket fill (${row.fe.kmOthers.toLocaleString("nl-BE")} km × ${fmtMoney(row.fe.fixedContributionPerKm).replace("€ ", "€")})`
                }
  ...
  return (
    <div>
      ...
      <div style={{ padding: "0 16px" }}>
        {step === 1 && (
          <RateStep
            title={lang === "nl" ? "① Vorig jaar, samengevat" : "① Last year, recap"}
            subtitle={
              lang === "nl"
                ? "Een eerlijk beeld voordat we beslissen."
                : "An honest picture before we decide."
            }
            onNext={() => setStep(2)}
            nextLabel={lang === "nl" ? "Verder — km schatten →" : "Next — estimate km →"}
          >
            <div
              style={{
                display: "grid",
            ...
            >
              <StatTile
                label={lang === "nl" ? "Totaal gereden" : "Total driven"}
                value={`${lastYearKm.toLocaleString("nl-BE")}`}
                suffix="km"
              />
              <StatTile
                label={lang === "nl" ? "Co-op gereden" : "Co-op driven"}
                value={`${a.trips
                  .filter((t) => !car.ownerIds.includes(t.who))
                  .reduce((s, t) => s + t.km, 0)
                  .toLocaleString("nl-BE")}`}
                suffix="km"
              />
              <StatTile
                label={lang === "nl" ? "Variabele kost/km" : "Variable cost/km"}
                value={fmtMoney(variablePerKm)}
                muted
              />
              <StatTile
                label={lang === "nl" ? "Vaste kost/jaar" : "Fixed cost/yr"}
            ...
            >
              {lang === "nl" ? (
                <>
                  Met <b>€ {car.prijs.toFixed(2).replace(".", ",")}/km</b> vorig jaar: de co-op-km
                  dekten{" "}
                  <b>
                    {Math.round(
                      ((a.trips
                        .filter((t) => !car.ownerIds.includes(t.who))
                        .reduce((s, t) => s + t.km, 0) *
                        (car.prijs - variablePerKm)) /
                        vasteTotal) *
                        100
                    )}
                    %
                  </b>{" "}
                  van de vaste kosten van {car.name}.
                </>
              ) : (
                <>
                  At <b>€ {car.prijs.toFixed(2).replace(".", ",")}/km</b> last year: co-op km
                  covered{" "}
                  <b>
                    {Math.round(
                      ((a.trips
        ...
        {step === 2 && (
          <RateStep
            title={
              lang === "nl"
                ? "② Hoeveel km verwacht je dit jaar?"
                : "② How many km do you expect this year?"
            }
            subtitle={
              lang === "nl"
                ? "Alleen co-op-km — niet je eigen gebruik."
                : "Co-op km only — not your own usage."
            }
            onBack={() => setStep(1)}
            onNext={() => setStep(3)}
          ...
          >
            ...
            <div style={{ textAlign: "center", padding: "10px 0 18px" }}>
              ...
              >
                {expectedKm.toLocaleString("nl-BE")}
              </div>
              <div
                style={{
                  fontFamily: paperB.fontMono,
              ...
              {[
                {
                  ...
                  v:
                    Math.round(
                      a.trips
                        .filter((t) => !car.ownerIds.includes(t.who))
                        .reduce((s, t) => s + t.km, 0) / 500
                    ) * 500 || car.expectedCoopKm,
                },
                {
                  l: lang === "nl" ? "−10%" : "−10%",
                  v: Math.round((car.expectedCoopKm * 0.9) / 500) * 500,
                },
                { l: lang === "nl" ? "Zelfde" : "Same", v: car.expectedCoopKm },
                {
                  l: lang === "nl" ? "+10%" : "+10%",
                  v: Math.round((car.expectedCoopKm * 1.1) / 500) * 500,
                },
              ].map((chip, i) => (
                <button
                  key={i}
                  onClick={() => setExpectedKm(chip.v)}
                  style={{
                    flex: "1 1 auto",
                    padding: "6px 10px",
        ...
        {step === 3 && (
          <RateStep
            title={lang === "nl" ? "③ Welk tarief vraag je?" : "③ What rate will you charge?"}
            subtitle={
              lang === "nl" ? "Zie live hoe vol je emmer wordt." : "See your bucket fill live."
            }
            onBack={() => setStep(2)}
            onNext={() => setStep(4)}
          ...
          >
            ...
            <div style={{ margin: "6px 0 16px" }}>
              ...
              >
                ...
                >
                  ...
                  >
                    {fmtMoney(Math.min(projectedRecovery, vasteTotal))}
                  </div>
                  <div
                    style={{
                      fontFamily: paperB.fontMono,
                  ...
                  >
                    {lang === "nl" ? "van" : "of"} {fmtMoney(vasteTotal)}{" "}
                    {lang === "nl" ? "eind jaar" : "by year end"}
                  </div>
                </div>
              </div>
              <div
              ...
              >
                <div>0%</div>
                <div>
                  {Math.round(projectedPct)}% {lang === "nl" ? "verwacht" : "projected"}
                </div>
                <div>100%</div>
              </div>
            </div>
            ...
            >
              € {proposedRate.toFixed(2).replace(".", ",")}
            </div>
            <div
              style={{
                textAlign: "center",
            ...
            >
              per km · {lang === "nl" ? "voorstel" : "proposed"}
            </div>
            ...
            >
              <span>€ 0,10</span>
              <span>
                {lang === "nl" ? "huidig:" : "now:"} € {car.prijs.toFixed(2).replace(".", ",")}
              </span>
              <span>€ 0,50</span>
            </div>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 12, flexWrap: "wrap" }}>
              {[
                ...
                <button
                  key={i}
                  onClick={() => setProposedRate(chip.v)}
                  style={{
                    flex: "1 1 auto",
                    padding: "7px 10px",
            ...
            >
              {projectedPct >= 100 ? (
                lang === "nl" ? (
                  <>
                    Met dit tarief vul je je emmer <b>volledig</b>. Overschot van{" "}
                    <b style={{ color: paperB.green }}>
                      {fmtMoney(projectedRecovery - vasteTotal)}
                    </b>{" "}
                    gaat naar je eigen zak.
                  </>
                ) : (
                  <>
                    This rate fills your bucket <b>completely</b>. Surplus of{" "}
                    <b style={{ color: paperB.green }}>
                      {fmtMoney(projectedRecovery - vasteTotal)}
                    </b>{" "}
                    goes to you.
                ...
                lang === "nl" ? (
                  <>
                    Dicht bij break-even:{" "}
                    <b style={{ color: paperB.amber }}>{fmtMoney(projectedShortfall)} tekort</b>.
                    Marge is krap — een grote reparatie kan je pijn doen.
                  </>
                ) : (
                  <>
                    Close to break-even:{" "}
                    <b style={{ color: paperB.amber }}>{fmtMoney(projectedShortfall)} short</b>.
                    Tight margin — a major repair could hurt.
                  </>
                )
              ) : lang === "nl" ? (
                <>
                  Let op:{" "}
                  <b style={{ color: paperB.accent }}>{fmtMoney(projectedShortfall)} tekort</b>. Je
                  draagt zelf een groot deel van de vaste kosten.
                </>
              ) : (
                <>
                  Warning:{" "}
                  <b style={{ color: paperB.accent }}>{fmtMoney(projectedShortfall)} short</b>.
                  You'll personally carry a big chunk of fixed costs.
                </>
              )}
            </div>
          </RateStep>
        ...
        {step === 4 && (
          <RateStep
            title={lang === "nl" ? "④ Bevestigen" : "④ Confirm"}
            subtitle={
              lang === "nl"
                ? "Dit legt het tarief vast voor dit jaar."
                : "This locks the rate for the year."
            }
            onBack={() => setStep(3)}
          >
            <div
            ...
            >
              ...
              />
              <ReceiptRow
                label={lang === "nl" ? "Verwachte co-op km" : "Expected co-op km"}
                value={expectedKm.toLocaleString("nl-BE") + " km"}
              />
              <Perf margin="8px 0" />
              <ReceiptRow
                label={lang === "nl" ? "Verwachte dekking" : "Projected recovery"}
                value={fmtMoney(Math.min(projectedRecovery, vasteTotal))}
              />
              <ReceiptRow
                label={lang === "nl" ? "Vaste kosten" : "Fixed costs"}
                value={fmtMoney(vasteTotal)}
              />
              <Perf margin="8px 0" />
              <ReceiptRow
                ...
                value={
                  (projectedPct >= 100 ? "+ " : "− ") +
                  fmtMoney(Math.abs(projectedRecovery - vasteTotal))
                }
                big
              ...
              <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
                <AdminButton>
                  ✓ {lang === "nl" ? "Vastleggen voor 2026" : "Lock in for 2026"}
                </AdminButton>
                <AdminButton variant="ghost" size="sm">
                  ✉ {lang === "nl" ? "Mail leden" : "Email members"}
                </AdminButton>
              </div>
            </div>
  ...
  return (
    ...
    >
      ...
      {(onBack || onNext) && (
        ...
        >
          {onBack && (
            <AdminButton onClick={onBack} variant="ghost" size="sm">
              ← {"Back"}
            </AdminButton>
          )}
          {onNext && (
            <div style={{ flex: 1 }}>
              <AdminButton onClick={onNext}>{nextLabel || "Next →"}</AdminButton>
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
        <div
          style={{
            ...
            fontWeight: 700,
            color: muted ? paperB.inkDim : paperB.ink,
            lineHeight: 1,
          }}
        >
          {value}
