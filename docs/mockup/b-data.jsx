    ...
    fixedCosts: [
      {
        ...
        description: "Omnium + aanhangwagen",
        amount: 1180,
      },
...
const PEOPLE = [
  { name: "Armando", username: "armando", password: "armando", isAdmin: false, k: 0.25, kl: 0.5 },
  { name: "Bouke", username: "bouke", password: "bouke", isAdmin: false, k: 0, kl: 0 },
  { name: "Joris", username: "joris", password: "joris", isAdmin: false, k: 0.25, kl: 0.5 },
  ...
  { name: "Susanna", username: "susanna", password: "susanna", isAdmin: false, k: 0.25, kl: 0.5 },
  { name: "Sverre", username: "sverre", password: "sverre", isAdmin: false, k: 0, kl: 0 },
  { name: "Tinne", username: "tinne", password: "tinne", isAdmin: false, k: 0.25, kl: 0.5 },
  { name: "Wim", username: "wim", password: "wim", isAdmin: false, k: 0.25, kl: 0.5 },
  { name: "Wouter", username: "wouter", password: "wouter", isAdmin: false, k: 0.25, kl: 0.5 },
  {
    name: "WouterLC",
    username: "wouterlc",
    password: "wouterlc",
...
function findConflict(car, from, to, excludeId) {
  ...
  return RESERVATIONS.find(
    (r) => r.id !== excludeId && r.status !== "rejected" && reservationOverlaps(r, candidate)
  );
}
...
function findKmGaps() {
  ...
  for (const car of CARS) {
    const trips = RECENT_TRIPS.filter((t) => t.car === car.short).sort((a, b) =>
...
function findSuspectFuel() {
  const flagged = [];
  for (const car of CARS) {
    const fills = RECENT_FUEL.filter((f) => f.car === car.short).sort((a, b) =>
      a.date.localeCompare(b.date)
    );
...
function fleetEconomics(car) {
  ...
  const breakEvenKm =
    fixedContributionPerKm > 0 ? Math.ceil(vasteTotal / fixedContributionPerKm) : Infinity;
  ...
  const status =
    pastBreakEven || projectedBurden <= 0 ? "green" : projectedBurdenPct <= 0.35 ? "amber" : "red";
  ...
  return {
    ...
    kmOwner,
    kmOthers,
    kmTotal: kmOwner + kmOthers,
    othersPaid,
    ownerVariableCost,
    variablePerKm,
    fixedContributionPerKm,
    vasteTotal,
    recoveredYTD,
    recoveredPct,
    breakEvenKm,
...
function yearlySettlement() {
  const rows = PEOPLE.map((p) => {
    ...
    const paidExp = RECENT_EXPENSES.filter((e) => e.who === p.name).reduce(
      (s, e) => s + e.bedrag,
      0
    );
