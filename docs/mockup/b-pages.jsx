        ...
        return (
          <div key={g.key}>
            ...
            >
              ...
              >
                {km.toLocaleString("nl-BE")} km · {fmtMoney(total)}
              </div>
            </div>
            <div style={{ padding: "0 16px" }}>
              {g.items.map((tr) => (
                <TripStrip key={tr.id} trip={tr} lang={lang} />
              ))}
            </div>
          </div>
        );
  ...
  return (
    <div style={{ background: paperB.paperDeep, minHeight: "100%", paddingBottom: 120 }}>
      ...
      <FilterRow
        value={carFilter}
        onChange={setCarFilter}
        options={[{ k: "all", l: t.filterAll }, ...CARS.map((c) => ({ k: c.short, l: c.short }))]}
      />
        ...
        return (
          <div key={g.key}>
            ...
            >
              ...
              >
                {liter.toFixed(1)} L · {fmtMoney(total)}
              </div>
            </div>
            <div style={{ padding: "0 16px" }}>
              {g.items.map((f) => (
                <FuelStrip key={f.id} fuel={f} lang={lang} />
              ))}
            </div>
          </div>
        );
...
function ReservePage({ lang, setLang, onNew }) {
  ...
  const resFor = (car, day) =>
    RESERVATIONS.find(
      (r) => r.car === car && r.status !== "rejected" && iso(day) >= r.from && iso(day) <= r.to
    );
  ...
  const inSelection = (carShort, day) => pick && pick.car === carShort && iso(day) === pick.from;
  ...
  return (
    <div style={{ background: paperB.paperDeep, minHeight: "100%", paddingBottom: 120 }}>
      ...
      >
        ...
        <span style={{ color: paperB.accent }}>
          ● {lang === "nl" ? "Klik 2 dagen om te reserveren" : "Click 2 days to reserve"}
        </span>
      </div>
          ...
          >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
              {days.map((d, i) => {
                ...
                let bg = paperB.paper,
                  fg = paperB.ink,
                  border = `1.5px solid ${paperB.paperDark}`;
                if (r) {
                  if (isPending) {
                ...
                return (
                  <div
                    ...
                    title={
                      r
                        ? `${r.who} — ${r.reason}${isPending ? " (in aanvraag)" : ""}`
                        : lang === "nl"
            ...
            {pick && pick.car === c.short && (
              <div
                style={{
                  ...
                  padding: "6px 10px",
                  background: paperB.paperDeep,
                  border: `1.5px dashed ${paperB.accent}`,
                  fontFamily: paperB.fontMono,
                  fontSize: 10,
                  letterSpacing: 1,
                  color: paperB.ink,
              ...
              >
                <span>
                  ● {lang === "nl" ? "Start gekozen" : "Start picked"}:{" "}
                  <b>{fmtDate(pick.from, lang)}</b> —{" "}
                  {lang === "nl" ? "klik einddatum" : "pick end date"}
                </span>
                <button
          ...
          {isPending && (
            <div
              style={{
                ...
                padding: "2px 6px",
                textTransform: "uppercase",
                border: `1.5px solid ${paperB.amber}`,
                color: paperB.amber,
              }}
            >
              {t.pendingBadge}
