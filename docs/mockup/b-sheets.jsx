    ...
    >
      ...
      <div
        ...
        style={{
          ...
          transition: torn
            ? "transform 0.45s cubic-bezier(0.5,0,0.8,1)"
            : dragY === 0
              ? "transform 0.22s"
              : "none",
        }}
      >
        ...
        >
          {progress > 0.85
            ? strings[lang].laatLos || "↓ Release ↓"
            : "≋ " + (strings[lang].veegOmlaag || "pull down to save") + " ≋"}
        </div>
      </div>
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "stretch" }}>
        ...
        >
          ...
          <span style={{ flex: 1, textAlign: "right" }}>
            {driver}
            {me?.k > 0 && <span style={{ color: paperB.accent, marginLeft: 4 }}>★</span>}
          </span>
          <span style={{ fontSize: 10, color: paperB.inkDim }}>{lockedDriver ? "🔒" : "▾"}</span>
        </div>
        <div
          style={{
            flex: 1,
      ...
      {driverOpen && !lockedDriver && (
        ...
        >
          ...
          >
            <span style={{ color: paperB.accent }}>★</span>{" "}
            {lang === "nl" ? "krijgt korting" : "has discount"}
          </div>
        </div>
      )}
      {lockedDriver && (
        ...
        >
          🔒{" "}
          {lang === "nl"
            ? "Jouw eigen naam — enkel admin kan voor anderen registreren"
            : "Your name — only admins can log on behalf of others"}
        </div>
      )}
    </div>
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ◎ {lang === "nl" ? "Kilometerstand" : "Odometer"}
          </div>
          ...
          {km > 500 && (
            ...
            >
              ▲{" "}
              {lang === "nl"
                ? "Lange rit — vanaf km 500 krijg je korting"
                : "Long trip — discount from km 500"}
            </div>
          )}
        </div>
        ...
        >
          ...
          <ReceiptRow
            label={t.tarief}
            value={`€ ${carObj.prijs.toFixed(2).replace(".", ",")} / km`}
          />
          {bd.baseKm > 0 && (
            <ReceiptRow
      ...
      >
        ...
        >
          ...
          <div>
            ...
            >
              {lang === "nl" ? "Nodig om verbruik te berekenen" : "Needed to compute consumption"}
            </div>
          </div>
        </div>
        ...
        {conflict ? (
          ...
          >
            ...
            >
              ...
              >
                {fmtDate(conflict.from, lang)}
                {conflict.from !== conflict.to && <> → {fmtDate(conflict.to, lang)}</>} ·{" "}
                {t[conflict.status]}
              </div>
            </div>
            <div
          ...
          <div
            style={{
              ...
              padding: "10px 14px",
              background: paperB.paper,
              border: `1.5px dashed ${paperB.green}`,
              fontFamily: paperB.fontMono,
              fontSize: 10,
              letterSpacing: 1.5,
              color: paperB.green,
          ...
          >
            ✓{" "}
            {lang === "nl" ? "Periode vrij — wacht op bevestiging" : "Period free — awaiting owner"}
          </div>
        )}
      </div>
    </SheetChrome>
...
function AvailabilityStrip({ car, from, to, lang }) {
  ...
  const resFor = (d) =>
    RESERVATIONS.find(
      (r) => r.car === car && r.status !== "rejected" && iso(d) >= r.from && iso(d) <= r.to
    );
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      ...
      >
        — {lang === "nl" ? "Beschikbaarheid" : "Availability"} —
      </div>
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      >
        <span>
          {label}
          {required && <span style={{ color: paperB.accent }}> *</span>}
        </span>
        {optional && (
...
Object.assign(window, {
  ...
  TearableSheet: SheetChrome,
  UnifiedTop: CompactMeta,
});
